...
[![CI](https://github.com/Var-666/AegisDesk/actions/workflows/ci.yml/badge.svg)](https://github.com/Var-666/AegisDesk/actions/workflows/ci.yml)
...
- HTTP Server 使用异步 Listener、Session strand 和有界 Handler 线程池，支持 Keep-Alive、背压和优雅关闭。
...
- [HTTP Server 性能验收报告](docs/http-server-performance-report.md)
...
│   ├── performance/        # 性能验收、Benchmark 和报告生成
│   ├── support/            # 测试数据构造器
│   └── unit/               # 单元测试和进程集成测试
└── runtime/
    └── logs/               # 本地运行日志
...
```mermaid
flowchart LR
    Client["并发客户端"] --> Listener["异步 Listener"]
    Listener --> Session["独立 HttpSession\nstrand + Keep-Alive"]
    Session --> Executor["有界 Handler 线程池"]
    Executor --> API["Agent API"]
    Executor -. "容量耗尽" .-> Busy["503 + Retry-After"]
```

网络 I/O 与业务 Handler 分离，单连接的读写由 strand 串行化，跨连接请求可以并发执行。连接数、在途请求数、请求大小、Keep-Alive 次数和空闲时间均有明确上限。

Apple M1、macOS arm64、Release 构建的本地回环验收结果如下。数据用于同一环境下的版本趋势比较，不代表公网或跨机器性能：

| 场景 | 并发连接 | 请求数 | 成功率 | 吞吐量 | P95 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 短连接 | 32 | 800 | 100% | 30,039 req/s | 1.23ms |
| Keep-Alive | 32 | 3,200 | 100% | 108,564 req/s | 0.36ms |
| Handler 模拟 5ms 工作 | 16 | 160 | 100% | 655 req/s | 25.06ms |

完整测试环境、P50/P95/P99 和验收口径见[性能验收报告](docs/http-server-performance-report.md)。CI 的 macOS、Linux Release 任务会重新生成报告，并将其写入 Job Summary、上传为 Artifact。
...
- HTTP Server 50 并发、持续 Keep-Alive 和 Handler 并行加速性能验收。
...
HTTP Server 性能门槛可以独立验收：

```bash
ctest --test-dir build -L performance --output-on-failure
```

生成当前机器的 Markdown Benchmark 报告：

```bash
cmake -S . -B build-performance \
  -DCMAKE_BUILD_TYPE=Release \
  -DBUILD_TESTING=ON
cmake --build build-performance --target aegis_http_benchmark --parallel
./build-performance/tests/aegis_http_benchmark \
...
- Linux TSan 检查。
- macOS 与 Linux Release Benchmark、Job Summary 和性能报告 Artifact。
