...
AegisDesk 是一个面向 macOS 与 Linux 的本地进程监督和桌面管理工具。项目采用 C++20 开发，由本地 Agent、Qt 6 桌面客户端和可控故障演示服务组成，支持服务注册、生命周期管理、运行指标采集、健康检查、告警和自动恢复。
...
cmake --build build-asan --target aegis_agent_core_tests --parallel
...
cmake --build build-tsan --target aegis_agent_core_tests --parallel
ctest --test-dir build-tsan --output-on-failure
```

> macOS 的 AddressSanitizer 不支持 LeakSanitizer。如运行环境默认启用泄漏检测，可设置 `ASAN_OPTIONS=detect_leaks=0`。Linux CI 保留泄漏检测。
