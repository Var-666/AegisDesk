...
#include "desktop/widgets/metric_chart_widget.h"
...
MetricsHistoryPanel::MetricsHistoryPanel(AgentClient& agent_client, QWidget* parent)
    ...
    status_value_ = ui::CreateValueLabel(this);
...
void MetricsHistoryPanel::Refresh() {
    ...
    agent_client_.GetMetricsHistory(
        ...
        [self, requested_service_id, generation](const std::optional<QList<ServiceMetricsSnapshot>>& history,
...
void MetricsHistoryPanel::ApplyHistory(const QList<ServiceMetricsSnapshot>& history) const {
    if (history.isEmpty()) {
...
void MetricsHistoryPanel::SetStatus(const QString& status) const {
