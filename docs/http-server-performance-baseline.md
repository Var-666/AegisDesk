...
本阶段新增独立的 `resilience` 测试组，不使用固定延迟作为跨机器性能门槛，重点验证高并发和故障条件下的正确性与资源回收：

- 8 个客户端分别在同一连接连续发送 20 个请求，共 160 次 Keep-Alive 请求全部成功；达到单连接请求上限后由服务端关闭连接。
- 32 个客户端发起请求的同时，8 个线程各重复调用 10 次 `RequestStop()`，验证停止幂等且不会死锁。
- 客户端只发送半个请求时，连接在读取超时后回收，服务端仍能处理后续正常请求。
- 客户端在 Handler 执行期间断开时，在途请求与 Session 均能回收，后续请求不受影响。
- 达到 `max_connections` 后拒绝额外连接；旧连接释放后，新连接可以正常进入。
- 同一 Server 实例连续执行 50 轮 `Start → Request → Stop → Wait`，每轮状态与计数归零，前后文件描述符数量保持稳定。
